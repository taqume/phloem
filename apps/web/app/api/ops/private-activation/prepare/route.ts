...
import { preparePrivateActivation } from "../../../../../lib/server/private-activation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prepared = await preparePrivateActivation({ company: input.company, sessionId: input.sessionId });
    return NextResponse.json(prepared, { headers: { "cache-control": "no-store" } });
  } catch (reason) {
    const message = reason instanceof Error ? reason.message : "PRIVATE activation preparation failed.";
