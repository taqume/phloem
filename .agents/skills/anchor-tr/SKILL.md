---
name: anchor-tr
description: TR Mock Anchor Testnet TRY/USDC discovery, SEP-10, SEP-12, SEP-38, SEP-6 deposit and withdrawal flows. Use for the hackathon Anchor integration and sandbox failure paths.
user-invocable: true
argument-hint: "[anchor task]"
---

# Stellar Mock Anchor Entegrasyon Skill'i

## Phloem security override

The upstream snippets below are protocol references. In Phloem, never load a company or provider secret with `Keypair.fromSecret`. Company signing uses Wallets Kit/Freighter, provider signing stays inside the server-side deterministic provider signer, and every live capability is re-discovered before use.
