...
use crate::{
    Error, PrivateSettlementBindingVerifier, PrivateSettlementBindingVerifierClient, Proof,
    VerificationKey,
};
...
fn client<'a>(env: &'a Env, vk: &VerificationKey) -> PrivateSettlementBindingVerifierClient<'a> {
    let contract_id = env.register(PrivateSettlementBindingVerifier, (vk.clone(),));
    PrivateSettlementBindingVerifierClient::new(env, &contract_id)
}

#[test]
fn verifies_the_real_private_settlement_binding_v1_proof() {
    let env = Env::default();
...
#[test]
fn rejects_replay_with_mutated_spp_output_or_audit_commitment() {
    let env = Env::default();
    let fixture = fixture(&env);
    let client = client(&env, &fixture.verification_key);

    let mut spp_mutation = fixture.public_inputs.clone();
    spp_mutation.set(
        5,
        spp_mutation.get_unchecked(5).add(&U256::from_u32(&env, 1)),
    );
    assert!(!client.verify(&fixture.proof, &spp_mutation));

    let mut audit_mutation = fixture.public_inputs.clone();
    audit_mutation.set(
        13,
        audit_mutation
            .get_unchecked(13)
    ...
    assert!(!client.verify(&fixture.proof, &audit_mutation));
}
