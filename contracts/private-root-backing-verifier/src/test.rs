...
use crate::{
    Error, PrivateRootBackingVerifier, PrivateRootBackingVerifierClient, Proof, VerificationKey,
};
...
fn client<'a>(env: &'a Env, vk: &VerificationKey) -> PrivateRootBackingVerifierClient<'a> {
    let contract_id = env.register(PrivateRootBackingVerifier, (vk.clone(),));
    PrivateRootBackingVerifierClient::new(env, &contract_id)
}

#[test]
fn verifies_the_real_private_root_backing_v1_proof() {
    let env = Env::default();
...
#[test]
fn rejects_mutated_root_spp_output_or_funding_amount() {
    let env = Env::default();
    let fixture = fixture(&env);
    let client = client(&env, &fixture.verification_key);

    for index in [1, 5, 6] {
        let mut mutation = fixture.public_inputs.clone();
        mutation.set(
            index,
            mutation.get_unchecked(index).add(&U256::from_u32(&env, 1)),
        );
        assert!(!client.verify(&fixture.proof, &mutation));
    }
