...
- Deterministic ExecutionGateway routing, generated TreasuryController binding, real RPC simulation adapter, contract-origin rejection evidence, and exact Agent Smart Account authorization-set checks: implemented. The PRIVATE invocation assembler now maps Supervisor delegation to `delegate_private_budget` and payment intent to `open_private_reservation`, while rejecting planner substitutions in session, actor, child identity, policy, amount, service, or offer metadata. The local proof planner and live submission adapters remain pending.
