...
- BudgetTransitionV1 now enforces 64-bit ranges, exact conservation, output type/shape rules, and context-bound commitments in Circom. A dedicated immutable-key Soroban verifier accepts the checked-in real Groth16 proof and rejects replay under a mutated context, malformed inputs, and non-canonical BN254 field values. The fixture uses a disclosed development-only single-contributor setup; wiring proof verification into TreasuryController and producing deployment-grade setup evidence remain pending.
