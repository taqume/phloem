...
- TreasuryController authority backbone: session creation, company-controlled Root materialization, real SAC custody funding, bounded STANDARD Root → Supervisor → child delegation, and PRIVATE reservation opening are implemented. PRIVATE opening requires the source Agent Smart Account, consumes its hidden BudgetNote through the configured BudgetTransition verifier, creates at most one hidden remainder note, records no plaintext amount/provider, binds one reservation-specific PaymentCommitmentKey, and updates the unresolved-reservation counters atomically. Exact authorization, conservation, policy, expiry, key-reuse, proof-rejection, and rollback behavior are covered by native contract tests.
...
- Contracts, circuits, wallet-authorized Phloem session funding, provider settlement, and final deployments: in progress.
